...
static SDL_Texture *current_font_texture = NULL;
...
void prepare_inline_font(const struct inline_font *font) {
  ...
  if (current_font_texture != NULL) {
    selected_font = current_font_texture;
    return;
  }
  ...
  current_font_texture = SDL_CreateTextureFromSurface(selected_renderer, surface);
  
  selected_font = current_font_texture;
}

void kill_inline_font(void) {
  SDL_DestroyTexture(current_font_texture);
